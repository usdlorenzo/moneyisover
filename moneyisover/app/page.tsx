...
const initialPositions: Record<BoxKey, Position> = {
  ...
  hero: { x: 120, y: 220 },
  courses: { x: 635, y: 115 },
  discussions: { x: 120, y: 445 },
  actions: { x: 1010, y: 285 },
  videos: { x: 645, y: 500 },
  reminders: { x: 645, y: 330 },
  calendar: { x: 1150, y: 560 },
};
...
export default function Home() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("moneyisover-box-positions");
  ...
  useEffect(() => {
    localStorage.setItem("moneyisover-box-positions", JSON.stringify(positions));
  }, [positions]);
  ...
  function startDrag(key: BoxKey, e: MouseEvent<HTMLDivElement>) {
    ...
    if (
      ["BUTTON", "INPUT", "TEXTAREA", "VIDEO"].includes(target.tagName)
    ) {
      return;
    }
    ...
    <main className="min-h-screen overflow-hidden bg-black text-white">
      ...
      <section className="relative z-10 flex min-h-screen items-center justify-center p-5">
        <div className="relative h-[850px] w-full max-w-[1450px] overflow-hidden rounded-[34px] border border-white/15 bg-black/5 shadow-[0_40px_140px_rgba(0,0,0,0.8)] backdrop-blur-[1px]">
          ...
          <div className="absolute left-[390px] top-9 w-[620px] rounded-full border border-white/10 bg-black/55 px-6 py-4 text-sm text-white/50">
          ...
          <Box pos={positions.time} onMouseDown={(e) => startDrag("time", e)} w="190px" h="85px">
            <div className="flex justify-between">
              <p className="text-3xl font-bold">{time}</p>
              <span>◴</span>
            </div>
            <p className="mt-1 text-[11px] text-white/60 capitalize">{date}</p>
          </Box>

          <Box pos={positions.weather} onMouseDown={(e) => startDrag("weather", e)} w="260px" h="120px">
          ...
          <Box pos={positions.prices} onMouseDown={(e) => startDrag("prices", e)} w="250px" h="145px">
            <div className="flex justify-between">
          ...
          <Box pos={positions.hero} onMouseDown={(e) => startDrag("hero", e)} w="360px" h="205px">
          ...
          <Box pos={positions.courses} onMouseDown={(e) => startDrag("courses", e)} w="330px" h="150px">
          ...
          <Box pos={positions.discussions} onMouseDown={(e) => startDrag("discussions", e)} w="500px" h="365px">
          ...
          <Box pos={positions.actions} onMouseDown={(e) => startDrag("actions", e)} w="350px" h="205px" p="p-2">
            <div className="grid grid-cols-4 gap-1">
              {actions.map(([icon, label]) => (
                <button
                  key={label}
                  className="h-[60px] rounded-xl border border-white/10 bg-black/25 text-[10px] hover:bg-white/10"
                >
          ...
          <Box pos={positions.videos} onMouseDown={(e) => startDrag("videos", e)} w="490px" h="310px">
            <div className="mb-3 flex justify-between">
          ...
          <Box pos={positions.reminders} onMouseDown={(e) => startDrag("reminders", e)} w="325px" h="145px">
          ...
          <Box pos={positions.calendar} onMouseDown={(e) => startDrag("calendar", e)} w="240px" h="190px">
            <div className="mb-3 flex justify-between">
              <button>‹</button>
...
function Box({
  ...
  return (
    <div
      ...
      className={`absolute rounded-[24px] border border-white/12 bg-black/55 ${p} shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-md`}
    >
      {children}
    </div>
  );
